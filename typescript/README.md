# Hyperworker — TypeScript Stack

TypeScript / Node.js variant of [Hyperworker](../README.md) — a collection of Claude Code skills that turn Claude into a multi-agent team lead.
...
```bash
cp /path/to/hyperworker/typescript/settings.json .claude/settings.json
```
...
cat /path/to/hyperworker/typescript/user-settings.json

# If ~/.claude/settings.json doesn't exist:
mkdir -p ~/.claude
cp /path/to/hyperworker/typescript/user-settings.json ~/.claude/settings.json
