# Hyperworker — Mobile App Stack

iOS Swift, Android Kotlin, and React Native variant of [Hyperworker](../README.md) — a collection of Claude Code skills that turn Claude into a multi-agent team lead.
...
- Runs platform-appropriate quality checks (build, lint, test)
- Runs automated UI tests if they exist (Maestro, XCUITest, Espresso)
...
- Agents determine the active platform(s) from the project structure (iOS, Android, React Native, or a combination)
- Agents do **not** manually open the simulator/emulator for ad-hoc QA — a dedicated final E2E QA agent handles that after all tasks are complete
- The E2E QA agent takes screenshots/video recordings and writes a proof summary for the Team Lead and human engineer to review
...
- Platform-specific toolchains as needed:
  - **iOS:** Xcode, `xcrun simctl`, CocoaPods or SPM
  - **Android:** Android Studio, Android SDK, Gradle
  - **React Native:** Node.js (LTS), `npm` or `pnpm`, Expo CLI (if using Expo)
...
cp -r /path/to/hyperworker/mobile-app/.claude/skills/* .claude/skills/
```
...
cp -r /tmp/hyperworker/mobile-app/.claude/skills/* .claude/skills/
...
```bash
cp /path/to/hyperworker/mobile-app/settings.json .claude/settings.json
```
...
cat /path/to/hyperworker/mobile-app/user-settings.json

# If ~/.claude/settings.json doesn't exist:
mkdir -p ~/.claude
cp /path/to/hyperworker/mobile-app/user-settings.json ~/.claude/settings.json
